...
WTS_URL=${WTS_OVERRIDE_URL:-"http://workspace-token-service.$NAMESPACE"}
echo $WTS_URL
...
run_sidecar() {
    while true; do
        ...
        for i in "${!IDPS[@]}"; do
            ...
            echo "check_for_new_manifests "$IDP_DATA_PATH" "$NAMESPACE" "$IDP" "$BASE_URL" "$TOKEN_JSON""
...
query_manifest_service() {
    ...
    if [[ $(jq -r '.error' <<< $resp) =~ 'log' ]]; then
        ...
        TOKEN_JSON[$IDP]=$(curl $WTS_URL/token/?idp=$IDP -H "Authorization: bearer ${ACCESS_TOKEN}" 2>/dev/null | jq -r '.token')
        resp=$(curl $URL -H "Authorization: bearer ${TOKEN_JSON[$IDP]}" 2>/dev/null)
    fi
}
